...
from molecule_encoder import Graph_encoder
import numpy as np 
import gpytorch
...
class ensemble_deep_gp(nn.Module):
    ...
    def __init__(self, args):
        ...
        for _ in range(num_encoder):
            ...
            if args.dataset == 'fsmol':
                ...
                elif 'GCN' in args.encode_method:
                    self.encoder_list.append(Graph_encoder.GCN_Graph(32, 1000, 1000, 7))
                elif 'GIN' in args.encode_method:
                    self.encoder_list.append(Graph_encoder.GIN_Graph(32, 1000, 1000, 7))
                elif 'GraphSAGE' in args.encode_method:
                    self.encoder_list.append(Graph_encoder.GraphSAGE(32, 1000, 1000, 7))
            elif args.dataset == 'pQSAR':
                if 'GAT' in args.encode_method: 
                    self.encoder_list.append(Graph_encoder.GAT_Graph(30, 200, 200, 5, 5))
                elif 'GCN' in args.encode_method:
                    self.encoder_list.append(Graph_encoder.GCN_Graph(30, 1000, 1000, 5))
                elif 'GIN' in args.encode_method:
                    self.encoder_list.append(Graph_encoder.GIN_Graph(30, 1000, 1000, 5))
                elif 'GraphSAGE' in args.encode_method:
                    self.encoder_list.append(Graph_encoder.GraphSAGE(30, 1000, 1000, 5))
        self.args = args
    ...
    def get_method_specific_data_format(self, x_support, y_support, x_query, y_query, device):
        x = x_support + x_query
        processed_y = torch.cat((torch.tensor(y_support), torch.tensor(y_query)), dim = 0).to(device)
        processed_x = Batch.from_data_list(x).to(device)
        num_data_support = len(x_support)
        num_data_query = len(x_query)
